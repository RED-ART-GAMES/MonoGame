...
#if PLAYSTATION4
            uint buttons;
            int packetNumber, xAxis, yAxis, wheel, tilt;
            Sce.PlayStation4.Input.Mouse.GetState((int)PlayerIndex.One, out packetNumber, out buttons, out xAxis, out yAxis, out wheel, out tilt);
            window.MouseState.LeftButton = (buttons & 0x1) != 0 ? ButtonState.Pressed : ButtonState.Released;
            window.MouseState.RightButton = (buttons & 0x2) != 0 ? ButtonState.Pressed : ButtonState.Released;
            window.MouseState.MiddleButton = (buttons & 0x4) != 0 ? ButtonState.Pressed : ButtonState.Released;
            window.MouseState.ScrollWheelValue += wheel;
            window.MouseState.X = MathHelper.Clamp(window.MouseState.X + xAxis, window.ClientBounds.Left, window.ClientBounds.Right);
            window.MouseState.Y = MathHelper.Clamp(window.MouseState.Y + yAxis, window.ClientBounds.Top, window.ClientBounds.Bottom);
#endif
