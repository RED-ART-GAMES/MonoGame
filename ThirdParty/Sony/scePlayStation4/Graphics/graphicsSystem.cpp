...
static const unsigned clear_p[] = {
	#include "..\clear_p.h"
};
static const unsigned clear_vv[] = {
	#include "..\clear_vv.h"
};
...
void GraphicsSystem::Initialize(int backbufferWidth, int backbufferHeight, TextureFormat backbufferFormat, DepthFormat depthFormat_)
{
	...
	for(uint32_t i=0; i<kDisplayBufferCount; ++i)
	{
		...
		tileSurfaceFailed = GpuAddress::computeSurfaceTileMode(Gnm::getGpuMode(), &tileMode, GpuAddress::kSurfaceTypeColorTargetDisplayable, format, 1) != SCE_OK;
#elif SCE_ORBIS_SDK_VERSION >= 0x01700081u // SDK Version 1.7
		...
		spec.m_width = backbufferWidth;
		spec.m_height = backbufferHeight;
		spec.m_numSlices = 1;
		...
		auto err = _displayBuffers[i].renderTarget.init(&spec);
		SCE_GNM_ASSERT(err == SCE_GNM_OK);

		auto sizeAlign = _displayBuffers[i].renderTarget.getColorSizeAlign();
		...
		if (_displayBuffers[i].hasDepthTarget)
		{
...
#if SCE_ORBIS_SDK_VERSION >= 0x03508041u // SDK Version 3.5
			tileSurfaceFailed = GpuAddress::computeSurfaceTileMode(Gnm::getGpuMode(), &depthTileMode, GpuAddress::kSurfaceTypeDepthOnlyTarget, depthFormat, 1) != SCE_OK;
#elif SCE_ORBIS_SDK_VERSION >= 0x01700081u // SDK Version 1.7
			...
			Gnm::DepthRenderTargetSpec dspec;
			dspec.init();
			dspec.m_zFormat = depthFormat.getZFormat();
			dspec.m_width = backbufferWidth;
			dspec.m_height = backbufferHeight;
			dspec.m_stencilFormat = kStencilFormat;
			dspec.m_tileModeHint = depthTileMode;
			dspec.m_numFragments = Gnm::kNumFragments1;
			dspec.m_minGpuMode = Gnm::getGpuMode();
			dspec.m_numSlices = 1;
			dspec.m_pitch = 0;
			dspec.m_flags.enableHtileAcceleration = kHtileEnabled ? 1 : 0;

			err = _displayBuffers[i].depthTarget.init(&dspec);
			SCE_GNM_ASSERT(err == SCE_GNM_OK);

			auto depthTargetSizeAlign = _displayBuffers[i].depthTarget.getZSizeAlign();
			stencilSizeAlign = _displayBuffers[i].depthTarget.getStencilSizeAlign();
			htileSizeAlign = _displayBuffers[i].depthTarget.getHtileSizeAlign();
#else
	...
	// Prepare the null texture.
	{
		...
		spec.m_format = Gnm::kDataFormatR8G8B8A8Unorm;
		spec.m_width = 2;
		spec.m_height = 2;
		spec.m_depth = 1;
		spec.m_numSlices = 1;
		...
		auto err = _nullTexture->init(&spec);
		SCE_GNM_ASSERT(err == SCE_GNM_OK);
		auto textureSizeAlign = _nullTexture->getSizeAlign();
											...
											uint32_t &desc3)
{
	...
	auto b = Gnmx::convertF32ToS2_4(0.0f);
										...
										float bottom)
{
	...
	sampler.setMipFilterMode(Gnm::kMipFilterModeLinear);
	sampler.setXyFilterMode(Gnm::kFilterModeBilinear, Gnm::kFilterModeBilinear);
	sampler.setWrapMode(Gnm::WrapMode::kWrapModeClampLastTexel, Gnm::WrapMode::kWrapModeClampLastTexel, Gnm::WrapMode::kWrapModeClampLastTexel);
