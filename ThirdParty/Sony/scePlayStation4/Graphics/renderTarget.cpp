...
void RenderTarget::Init(TextureFormat format_, int32_t width, int32_t height, int32_t slices, DepthFormat depthFormat_, bool isCubemap)
{
	...
	GpuAddress::computeSurfaceTileMode(Gnm::getGpuMode(), &tileMode, GpuAddress::kSurfaceTypeColorTarget, format, 1);
#else
	...
	spec.m_numSlices = slices;
	spec.m_pitch = 0;
	//spec.m_flags = ???

	auto err = renderTarget->init(&spec);
	SCE_GNM_ASSERT(err == SCE_GNM_OK);

	auto renTargetSizeAlign = renderTarget->getColorSizeAlign();
	...
	dspec.m_numSlices = slices;
	dspec.m_pitch = 0;
	//dspec.m_flags = ???

	err = _depthTarget->init(&dspec);
	SCE_GNM_ASSERT(err == SCE_GNM_OK);

	auto depthTargetSizeAlign = _depthTarget->getZSizeAlign();
	stencilSizeAlign = _depthTarget->getStencilSizeAlign();
...
void RenderTarget::InitFromTexture2D(Texture* texture, DepthFormat depthFormat_)
{
	...
	dspec.init();
	dspec.m_zFormat = depthFormat.getZFormat();
	dspec.m_width = colorWidth;
	dspec.m_height = colorHeight;
	dspec.m_stencilFormat = kStencilFormat;
	dspec.m_tileModeHint = depthTileMode;
	dspec.m_numFragments = Gnm::kNumFragments1;
	dspec.m_minGpuMode = Gnm::getGpuMode();
	...
	auto err = _depthTarget->init(&dspec);
	SCE_GNM_ASSERT(err == SCE_GNM_OK);
