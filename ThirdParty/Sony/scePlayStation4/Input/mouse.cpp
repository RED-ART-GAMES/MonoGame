	...
	int32_t &tilt)
{
	...
	packetNumber = states[playerIndex].PacketNumber;
	buttons = states[playerIndex].Buttons;
	xAxis = states[playerIndex].XAxis;
	yAxis = states[playerIndex].YAxis;
	wheel = states[playerIndex].Wheel;
	tilt = states[playerIndex].Tilt;

	return states[playerIndex].IsConnected;
}
