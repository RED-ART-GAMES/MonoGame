...
NpScoreRankings::NpScoreRankings(size_t arrayNum, bool needRanks, bool needComments, bool needGameInfo)
{
	...
	_rank = needRanks ? mem::alloc_array<SceNpScoreRankDataA>(arrayNum, 8) : NULL;
...
{
	if (_rank)
		return _rank[_index].onlineId.data;
	return NULL;
}

SceNpAccountId NpScoreRankings::GetUserAccountIdAtIndex()
{
	if (_rank)
		return _rank[_index].accountId;
														...
														NpScoreRankings *results)
{
	...
	auto error = sceNpScoreGetRankingByRangeA(	_requestId, 
												boardId, 
												startSerialRank,
												results->_rank,
												results->_arrayNum * sizeof(SceNpScoreRankDataA),
												results->_comment,
												   ...
												   NpScoreRankings* results)
{
	...
	auto error = sceNpScoreGetFriendsRankingA(	_requestId, 
												boardId, 
												includeSelf,
												results->_rank,
												results->_arrayNum * sizeof(SceNpScoreRankDataA),
												...
												NpScoreRankings *results)
{
	...
	SceNpScorePlayerRankDataA rankData;
	SceNpAccountId id;
	auto error = sceNpGetAccountIdA(userId, &id);
#else
...
#if SCE_ORBIS_SDK_VERSION >= 0x04508111u // SDK Version 4.5
	error = sceNpScoreGetRankingByAccountId(_requestId, 
#else
	...
	error = sceNpScoreGetRankingByRangeA(	_requestId, 
												boardId, 
												startRank,
												results->_rank,
												results->_arrayNum * sizeof(SceNpScoreRankDataA),
...
void* NpScoreRequest::GetGameData(				SceNpScoreBoardId boardId,
												SceNpAccountId accountId,
												CS_OUT size_t *totalSize,
												CS_OUT NpCommunityError *error)
{
	size_t local_totalSize = 0;
	int ret = 0;
	void *ptr = NULL;

	ret = sceNpScoreGetGameDataByAccountId(_requestId, boardId, accountId, &local_totalSize, 0, ptr, NULL);
	ptr = malloc(local_totalSize);

	if (ret < 0)
	{
		*totalSize = 0;
		*error = (NpCommunityError)ret;
		return ptr;
	}
	else
	{
		ret = sceNpScoreGetGameDataByAccountId(_requestId, boardId, accountId, &local_totalSize, local_totalSize, ptr, NULL);
	}
...
NpCommunityError NpScoreRequest::RecordData(	SceNpScoreBoardId boardId,
												SceNpScoreValue score,
												const char *data)
{
	size_t total_size = data ? strlen(data) : 0;
	size_t send_size = total_size;
	auto error = sceNpScoreRecordGameData(_requestId, boardId, score, total_size, send_size, data, NULL);
	if (error < 0)
		return (NpCommunityError)error;
	else
		return NpCommunityError::Ok;
...
NpScoreTitleContext* NpScoreTitleContext::Create(UserServiceUserId userId, CS_OUT NpResult *error)
{
	...
	memcpy(&context->_npId, &npId, sizeof(npId));
...
{	
	...
	// Create the context.
#if SCE_ORBIS_SDK_VERSION >= 0x04508111u // SDK Version 4.5
	auto error = sceNpScoreCreateNpTitleCtxA(npServiceLabel, _npId);
#else
