...
Compiler::SourceFile* OpenFileCallback(
	...
	const Compiler::OptionsBase *compileOptions,
#ifdef USING_WAVE
	void *userData,
#endif
	const char **errorString)
{
	auto sourceFile = (Compiler::SourceFile*)userData;
	return sourceFile;
}
...
bool ShaderCompiler::Compile(String^ filePath, String^ code, String^ entryPoint, String^ profile, bool debug, [Out]String^ %buildOutput)
{
	...
	// We might loose a little performance, but force shaders
	// to work for both NEO or BASE PS4s for simplicity.
	options.hardwareTarget = Compiler::kHardwareTargetCommon;
...
ElementDesc^ BufferDesc::GetElement(int index)
{
	...
	element->TypeName = gcnew String(typeName);
